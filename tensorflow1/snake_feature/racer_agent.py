...
class A3CAgent:
    ...
    def train(self):
        ...
        while True:
            ...
            if len(self.memory) > self.train_start:
                a_loss, c_loss = self.train_batch()
                actor_loss += abs(a_loss)
                critic_loss += abs(c_loss)
                
                T += 1
                if T >= self.actor_rate:
                    for agent in agents:
                        agent.update_local_model()
                    print('Actor loss:', actor_loss/T, 'Critic loss:', critic_loss/T)
                    actor_loss = 0
                    critic_loss = 0
                    T = 0
    ...
    def train_batch(self):
        ...
        states = np.zeros([0] + list(self.state_size))
        actions = np.zeros([0, self.action_size])
        adv_actor = np.zeros([0, 1])
        adv_critic = np.zeros([0, 1])
        ...
        for transition in batch:
            ...
            discounted = np.zeros_like(values[:-1])
            ...
            adv_actor = np.append(adv_actor, advantage, axis=0)
            adv_critic = np.append(adv_critic, discounted, axis=0)
        ...
        while True:
            ...
            while not done:
                ...
                if self.t >= self.t_max or done:
                    self.t = 0
                    self.upload_sample(next_history, done)
