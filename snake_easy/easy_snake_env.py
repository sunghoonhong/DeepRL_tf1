...
class Game:
    ...
    def create_goal(self):
        for i, g in enumerate(self.goal_map):
            if g == 0 :
                goal = Goal()
                
                goal.rect.topleft = ((i//3 + 1)*100, (i%3+1)*100)
                
                self.goal_map[i] = 1
        # if not self.goals:
        #     goal = Goal()
        #     goal.rect.topleft = (100, 100)
            # while True:
            #     goal.rect.topleft = (
    ...
    def collision(self):
        ...
        if body_collision:
            ...
            goal = pg.sprite.spritecollide(self.snake.head, self.goals, True)
            if goal:
                pos = goal[0].rect.topleft
                x = (pos[0]//100-1)
                y = (pos[1]//100-1)
                # print(pos, x, y)
                self.goal_map[x*3+y]=0
                self.score += 1
                info = 'goal'
